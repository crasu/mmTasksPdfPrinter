module.exports = {
  progressTask: function (jiraCliPath, jiraUrl, jiraUser, jiraPass, jiraProject, jiraTaskId, jiraTaskStep, jiraTaskAssignee, callback) {
    if(jiraCliPath && jiraUrl && jiraUser && jiraPass && jiraProject && jiraTaskId && jiraTaskStep && jiraTaskAssignee && callback) {
      ...
      var errorCallback = function (nr) {
        return function (err) {
          ...
          if(nr === 0) {
            callback(null, {
              jiraTask: jiraTaskId,
              stepName: jiraTaskStep,
              user: jiraTaskAssignee
            });
          } else if(nr === 1) {
            outputCallback(1);
          }
      ...
      var outputCallback = function (nr) {
        if(nr === 0) {
          return function (data) {
            /*log data.toString()*/
            process = cp.spawn(jiraCliPath, ['-a', 'progressIssue', '-s', jiraUrl, '-u', jiraUser, '-p', jiraPass, '--project', jiraProject, '--issue', jiraProject + '-' + jiraTaskId, '--step', jiraTaskStep]);
          ...
          return function (data) {
            ...
            process.stderr.on('data', errorCallback(2));
            process.stdout.once('data', outputCallback(2));
          };
        } else if(nr === 2) {
          return function (data) {
            /*log data.toString()*/
      ...
      var process = cp.spawn(jiraCliPath, ['-a', 'updateIssue', '-s', jiraUrl, '-u', jiraUser, '-p', jiraPass, '--project', jiraProject, '--issue', jiraProject + '-' + jiraTaskId, '--assignee', jiraUser]);
